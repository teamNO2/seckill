...
public class ClickController{
    ...
    public Callable<GenericResponse> clickRob(@PathVariable("sceneId") Integer sceneId,@PathVariable Integer managerId){
        ...
        if (day.after(date)) {
            ...
            if(sceneService.selectById(sceneId).getSceneStarttime().split(" ",1).equals(sceneService.selectById((sceneId+1)).getSceneStarttime().split(" ",1))){
                System.out.println();
            ...
            return () -> GenericResponse.success("click", "已经参加活动", joined);
        }
        return null;
    }
}
